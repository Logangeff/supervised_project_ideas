...
def _train_text_model(
    ...
    if use_claim_prob:
        X_train = sparse.hstack([X_train_text, sparse.csr_matrix(splits["train"]["claim_prob"].to_numpy()[:, None])], format="csr")
        X_validation = sparse.hstack([X_validation_text, sparse.csr_matrix(splits["validation"]["claim_prob"].to_numpy()[:, None])], format="csr")
        X_test = sparse.hstack([X_test_text, sparse.csr_matrix(splits["test"]["claim_prob"].to_numpy()[:, None])], format="csr")
    else:
    ...
    model_bundle = {
        "model_type": "claim_aware" if use_claim_prob else "all_text",
        "model_name": model_name,
        "vectorizer": vectorizer,
        "classifier": best_classifier,
        "uses_claim_prob": use_claim_prob,
    }
    ...
    figure_path = STAGE2_CLAIM_AWARE_FIGURE_PATH if use_claim_prob else STAGE2_ALL_TEXT_FIGURE_PATH
    save_confusion_matrix_figure(
        y_test,
        test_predictions,
        STAGE2_LABEL_NAMES,
...
def run_stage2_models() -> dict[str, object]:
    ...
    split_frames = {
        split_name: claim_scored_dataset[claim_scored_dataset["event_id"].isin(split_frame["event_id"])]
    ...
    all_text_model, all_text_summary = _train_text_model("all_text", split_frames, use_claim_prob=False)
    claim_aware_model, claim_aware_summary = _train_text_model("claim_aware", split_frames, use_claim_prob=True)
    joblib.dump(all_text_model, STAGE2_ALL_TEXT_MODEL_PATH)
    joblib.dump(claim_aware_model, STAGE2_CLAIM_AWARE_MODEL_PATH)
    write_json(
        STAGE2_TEXT_SUMMARY_PATH,
        {"all_text": all_text_summary, "claim_aware": claim_aware_summary},
    )
    return {
...
def _predict_stage2_model(model_bundle: dict[str, object], frame: pd.DataFrame) -> list[int]:
    ...
    if model_bundle["uses_claim_prob"]:
        X = sparse.hstack([X_text, sparse.csr_matrix(frame["claim_prob"].to_numpy()[:, None])], format="csr")
    else:
        X = X_text
...
def run_stage2_evaluate() -> dict[str, object]:
    ensure_project_dirs()
    set_global_seed(SEED)
    if not STAGE2_MARKET_MODEL_PATH.exists() or not STAGE2_ALL_TEXT_MODEL_PATH.exists() or not STAGE2_CLAIM_AWARE_MODEL_PATH.exists():
        run_stage2_models()

    base_dataset = load_stage2_dataset()
    dataset = _ensure_claim_scores(base_dataset)
    split_frames = load_stage2_splits()
    split_frames = {
