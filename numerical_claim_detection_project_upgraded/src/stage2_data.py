...
def _has_suspicious_price_discontinuity(prev_day_price: float, curr_day_price: float, next_day_price: float) -> bool:
    prev_to_curr_ratio = curr_day_price / prev_day_price
    curr_to_next_ratio = next_day_price / curr_day_price
    return (
        prev_to_curr_ratio > 2.0
        or prev_to_curr_ratio < 0.5
        or curr_to_next_ratio > 2.0
        or curr_to_next_ratio < 0.5
    )
...
def _build_filtered_stage2_frame() -> tuple[pd.DataFrame, dict[str, int]]:
    ...
    for file_path in sorted(raw_dir.glob("*.json.xz")):
        for article in _load_year_file(file_path):
            ...
            if _has_suspicious_price_discontinuity(prev_day_price, curr_day_price, next_day_price):
                counters["dropped_suspicious_price_jump"] += 1
                continue
            ...
            next_day_return = (next_day_price - curr_day_price) / curr_day_price
            ...
            records.append(
                {
                    ...
                    "abs_same_day_return": abs(same_day_return),
                    "next_day_return": next_day_return,
                    "abs_next_day_return": abs(next_day_return),
...
def load_stage2_dataset() -> pd.DataFrame:
    ...
    required_columns = {"next_day_return", "abs_next_day_return", "abs_same_day_return"}
